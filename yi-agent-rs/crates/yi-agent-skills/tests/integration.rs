...
use yi_agent_skills::{SkillScope, SkillsService, install_system_skills};
...
fn bundled_skills_install_and_discover_correctly() {
    ...
    assert!(
        catalog.contains("skill-creator"),
        "catalog missing skill-creator"
    );
    assert!(
        catalog.contains("skill-installer"),
        "catalog missing skill-installer"
    );
}
...
fn render_catalog_truncates_excessively_large_catalog() {
    ...
    let service = SkillsService::new(vec![(tmp.path().to_path_buf(), SkillScope::User)]);

    let full_size = service.full_catalog_size();
    assert!(
        full_size > 500,
        "full catalog should be > 500 bytes, got {}",
        full_size
    );
    ...
    assert!(
        catalog.len() <= small_budget,
        "catalog len {} exceeds budget {}",
        catalog.len(),
        small_budget
    );
